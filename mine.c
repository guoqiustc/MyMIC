		  ...
		  int *P_map, int p, int x, double *score)
{
  ...
  /*
	 * c is an array contained the prefix sum of P_map
	 */
  /* compute c */
  ...
  if (c == NULL)
    goto error_c;
  /*
	 * cumhist is the cumulative histgram matrix
	 */
...
mine_score *mine_compute_score(mine_problem *prob, mine_parameter *param)
{
  ...
  for (i=0; i<score->n; i++)
    ...
    for (j=0; j<prob->n; j++)
		...
		ret = OptimizeXAxis(yy, xy, prob->n, Q_map, q, P_map, p, score->m[i]+1, M_temp);
		if (ret)
			goto error_0;
    ...
    for (j=0; j<score->m[i]; j++)
		{
			...
			if (((score->M[i][j]+delta) >= ((1.0 - eps) * mic)) && (log_xy < score_min))
				score_min = log_xy;
		}
